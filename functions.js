...
function proxy(url, callback){
      ...
      $.getJSON('https://script.google.com/macros/s/AKfycbwHUbAAtF_tFFrdBiZon6gKknEDYyxMosA0NpjSLPZyIBy8jqSAZLAOGeCAHgiHA2er/exec?url=' +
          encodeURIComponent(url) + '&callback=?', function(data){
            // console.log("proxu data: "+data);
            callback(data);
          });
}

function imageProxy(url, imgContainer){
  $.get("https://script.google.com/macros/s/AKfycbwFisAHoNikKL667ID25N_2zEDdy0ueaVu9NyDkvHpUFZwro5ZZV5zs7HsK62Mf6p8/exec?url="+
  encodeURIComponent(url), function(data){
    // console.log(data);
    $(imgContainer).css("background-image", "url("+data+")");
  });
}
...
function loadKofiWidget(kofiID) {
